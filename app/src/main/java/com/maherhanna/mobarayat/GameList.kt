...
fun GameList(viewModel: FootballViewModel, league: League) {
    ...
    LazyColumn {
        items(games) { game ->
            val prediction = game.predictions?.find { it.userId == viewModel.userId }
            var homeScore by remember { mutableStateOf(prediction?.homeScore?.toString() ?: "") }
            var awayScore by remember { mutableStateOf(prediction?.awayScore?.toString() ?: "") }
